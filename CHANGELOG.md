...
- **Provider icons stopped appearing on recent Open WebUI builds** — OWUI migrated its `Models` table methods (`get_model_by_id`, `insert_new_model`, `update_model_by_id`) to `async` coroutines. `_sync_model_icons` called them synchronously, so every read returned an un-awaited coroutine and every write was a silent no-op — no icon ever reached the database (every model showed OWUI's default). Added `_resolve_owui_call`, which runs a returned coroutine to completion in a dedicated thread (the sync `pipes()` executes inside OWUI's running event loop, so `asyncio.run` on the current loop is not possible) and passes synchronous results through unchanged, so the pipe works on both old (sync) and new (async) OWUI. Confirmed against `open-webui:main`.
- **Default pipe-model icon not recognised as overwritable** — current OWUI assigns `/static/favicon.png` as the default icon for manifold sub-models, which `_is_owui_managed_icon` treated as a user-set custom icon and refused to overwrite. `/static/` paths are now recognised as managed defaults, so provider icons replace them.
